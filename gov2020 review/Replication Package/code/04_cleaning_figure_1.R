...
##load data 
chips <- read_csv("data/chips.csv") %>% 
  filter(law == "CHIPS")
cd_pres_vote <- read_excel("data/cd_pres_vote.xlsx") %>% 
...
save(cd_pres_vote, file = "data/cd_chips.rdata")
...
ggsave(filename = "extension/pt_states.png", plot = pt_states, width = 10, height = 6, bg='#ffffff')
